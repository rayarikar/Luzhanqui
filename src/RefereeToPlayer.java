...
/**
 * This class reads the input message from the console and checks whether the
 * message is a valid message or not
 * 
 * @author Rohan
 * 
 */
public class RefereeToPlayer {
	...
	/**
	 * Matches the input string with the pattern. If the match is found it
	...
	/**
	 * Checks the passed message with the expected message and returns true if
	 * the message is valid, else false
	 * 
	 * @param message
	 * @return boolean
	 */
	public List processRefereeMessage(String message) {
		...
		// return false if message does not have opening and closing 
		// parenthesis
		if (messageBuilder.charAt(0) != LEFT_PARENTHESIS
				|| messageBuilder.charAt(messageBuilder.length() - 1) 
				!= RIGHT_PARENTHESIS)
			return emptyList;
		// remove the parenthesis and process message
		messageBuilder.deleteCharAt(0);
		messageBuilder.deleteCharAt(messageBuilder.length() - 1);
		return checkForMessageType(new StringBuilder(messageBuilder
				.toString().trim()));
	}
	...
	/**
	 * This helper method validates the message and checks for strings go
	 * <which> time/move <seconds>
	 * 
	 * @param innerMessage
	 * @return boolean
	 */
	...
	/**
	 * This helper method takes the <seconds> input in string form and 
	 * checks for the input having valid digits
	 * 
	 * @param secondsInput
	 * @return boolean
	 */
	...
	/**
	 * This method checks if the passed input is a valid digit or not 
	...
	private List checkForGameEndMessage(StringBuilder innerMessage) {
		...
		if (Integer.parseInt(innerMessage.toString().charAt(0) + "") <= 2
				&& Integer.parseInt
						(innerMessage.toString().charAt(0) + "") >= 0) {
			returnList.add(GAME_END);
			returnList.add(Integer.parseInt(innerMessage.toString()
					.charAt(0) + ""));
			return returnList;
		}
		return emptyList;
	...
	private List checkForGoMessage(StringBuilder innerMessage) {
		...
		if (Integer.parseInt(innerMessage.toString().charAt(0) + "") == 1
				|| Integer
						.parseInt
						(innerMessage.toString().charAt(0) + "") == 2) {
			returnList.add(GO);
			returnList.add(Integer.parseInt(innerMessage.toString()
					.charAt(0) + ""));
			return returnList;
	...
	private List checkForFlagMessage(StringBuilder innerMessage) {
		...
		String player = ""
				+ Integer.parseInt
				(innerMessage.toString().charAt(0) + "");
		innerMessage.deleteCharAt(0);
		innerMessage = Utility.removeStartBlankChars(innerMessage);
		String position = innerMessage.toString().substring(0,
				innerMessage.toString().trim().length());
	...
	private List checkForIllegalMessage(StringBuilder innerMessage) {
		...
		if (modifiedMessage.charAt(0) == LEFT_PARENTHESIS
				&& modifiedMessage.charAt(modifiedMessage.length() - 1) 
				== RIGHT_PARENTHESIS) {
			List illegalList = checkForPositions(modifiedMessage
					.toString().trim()
					.substring(1, modifiedMessage.length() - 1));
	...
	private List checkForPositions(String innerMessage) {
		List<String> positions = Utility
				.extractPositionsFromString(innerMessage);
		if (positions.size() < 2)
			return emptyList;
	...
	private List checkForOutcomeMessage(StringBuilder innerMessage) {
		...
		// if it is <move>
			...
			List moveList = checkForPositions(innerMessage.toString()
					.trim().substring(1, innerMessage.length() - 1));
			if (moveList.size() == 3)
				return moveList;
			else
				return emptyList;
	...
	private List checkMoveMessage(StringBuilder innerMessage) {
		if (innerMessage.charAt(0) == LEFT_PARENTHESIS
				&& innerMessage.charAt(innerMessage.length() - 1) 
				== RIGHT_PARENTHESIS)
			return checkForPositions(innerMessage.toString().trim()
